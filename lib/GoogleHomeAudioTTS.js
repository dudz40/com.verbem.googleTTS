...
class GoogleHomeAudioTTS {
    ...
    setRestoreVolume(restore) {
        console.log('restore volume', restore);
    ...
    playOnDevice(url) {
        ...
        let closeClient = function () {
            console.log('closing client', originalVolume);
            if (originalVolume) {
        ...
        client.connect(`https://${that.ip}:${that.port}`, () => {
            volume && client.getVolume((err, value) => {
                originalVolume = value;
                if (value.muted && that.sendToMuted) client.setVolume({ level: volume }, ( /*err, vol*/) => { });
            });

            console.log('connected, launching app ...', that.ip, url);
